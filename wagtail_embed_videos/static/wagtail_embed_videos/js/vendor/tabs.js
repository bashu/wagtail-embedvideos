/*
Vendored from Wagtail 3.0.x: client/src/includes/tabs.js
https://github.com/wagtail/wagtail/blob/stable/3.0.x/client/src/includes/tabs.js

Wagtail compiles this into its own webpack bundles (wagtailadmin/js/vendor.js) and
never exposes it as a public global, so third-party static JS (like the rest of this
package) has no way to import or call it. Rather than reimplement a partial subset of
its behavior, this is a verbatim copy of the upstream class, with only the ES module
`import`/`export` statements at the top/bottom replaced by a plain namespaced global
assignment so it can be loaded as an ordinary <script> with no build step.

If you're updating this file, diff it against the upstream source above and keep
everything between the two markers unchanged so `Tabs` stays behavior-identical to
Wagtail's own implementation.
*/
(function (window) {
    'use strict';

    // ---- verbatim copy of upstream client/src/includes/tabs.js starts here ----
    ...
    const initTabs = (tabs = document.querySelectorAll('[data-tabs]')) => {
    ...
    // ---- verbatim copy of upstream client/src/includes/tabs.js ends here ----

    window.WagtailEmbedVideosTabs = {
        Tabs: Tabs,
        initTabs: initTabs,
    };
})(window);

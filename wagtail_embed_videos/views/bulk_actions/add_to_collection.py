...
from wagtail_embed_videos.views.bulk_actions.embed_video_bulk_action import (
    EmbedVideoBulkAction,
)
...
class CollectionForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["collection"] = forms.ModelChoiceField(
            queryset=EmbedVideoBulkAction.permission_policy.collections_user_has_permission_for(
                user,
                "add",
            ),
        )
...
class AddToCollectionBulkAction(EmbedVideoBulkAction):
    ...
    template_name = (
        "wagtail_embed_videos/bulk_actions/confirm_bulk_add_to_collection.html"
    )
    action_priority = 30
    ...
    def execute_action(cls, embed_videos, collection=None, **kwargs):
        ...
        num_parent_objects = (
            ...
            .objects.filter(pk__in=[obj.pk for obj in embed_videos])
